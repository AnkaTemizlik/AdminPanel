...
const DataTable = React.memo((props) => {
  ...
  const editing = props.editing ||
    gridOptions.editing || { enabled: false, mode: 'popup', useIcons: true };
  const pager = gridOptions.pager || { visible: true };
  const paging = gridOptions.paging || { enabled: true, defaultPageSize: 10 };
  const sorting = gridOptions.sorting || {
    enabled: true,
    mode: 'multiple',
    showSortIndexes: true,
    default: { selector: keyFieldName, desc: true },
  };
  ...
  useEffect(() => {
    console.purple('DataTable dataSource', dataSource);
  }, [dataSource]);
  ...
  return (
    <>
      <DataGrid
        ...
        style={{
          maxHeight: gridOptions.maxHeight || 760,
        ...
        columnAutoWidth={true}
        onRowUpdating={(o) => {
          console.success('DataTable onRowUpdating', o, dataSource);
          if (dataSource.passWholeRowOnUpdate == true)
            o.newData = { ...o.oldData, ...o.newData };
        }}
      >
        {/* <StateStoring enabled={true} type="localStorage" storageKey={"gridStorage_" + props.id} />  */}
        {gridOptions.grouping && (
          <GroupPanel visible={gridOptions.grouping.visible} />
        )}
        {gridOptions.grouping && (
          <Grouping autoExpandAll={gridOptions.grouping.autoExpandAll} />
        )}
        {gridOptions.grouping && gridOptions.grouping.visible == true && (
        ...
        <LoadPanel
          enabled={!Array.isArray(dataSource)}
          shadingColor='rgba(0,0,0,0.05)'
          shading={true}
        />

        {gridOptions.allowFilter !== false && <HeaderFilter visible={true} />}

        {gridOptions.allowFilter !== false && <FilterRow visible={true} />}

        {gridOptions.allowFilter !== false && <FilterPanel visible={true} />}
        ...
        {editing && (
          ...
          >
            ...
            <Form>
              {_.orderBy(columns, ['editIndex'], ['asc'])
                .map((c, i) => {
                  ...
                  //console.info('allowEditing', c, c.name, item);
          ...
          <Column type='buttons' width={70}>
            ...
            {actions &&
              actions
                .filter(
                  (a) =>
                    props.editing.allowEditing && a.showInEditColumn == true
                )
                .map((a) => {
                  return (
                    <Button
        ...
        {columns.map((c, i) => {
          let defaultSortOrder = c.defaultSortOrder
            ? c.defaultSortOrder
            : undefined;
          ...
          return (
            <Column
              ...
              //alignment={c.alignment || c.dataType == "string" ? "left" : "right"}
              caption={c.caption}
              ...
              cellRender={
                c.type == 'image'
                  ...
                  ? (cell) => (
                      <div
                        className='dx-colorbox-color-result-preview'
                        style={{
              ...
              editCellComponent={
                c.editWith
                  ? c.editWith.type == 'table'
                    ...
                    : c.editWith.type == 'tagBox'
                    ? (editCellProps) => (
                        <TagBoxComponent {...editCellProps} column={c} />
                      )
                    : null
                  : null
              }
            >
              {c.required === true && <RequiredRule />}
              {typeof c.customStore == 'function' ? (
                <Lookup
                  dataSource={(o) => c.customStore(o)}
                  valueExpr={c.data.valueExpr}
                  displayExpr={c.data.displayExpr}
                />
              ) : c.customStore ? (
                <Lookup
                  dataSource={{
                ...
                c.editWith.type == 'tagBox' ? null : c.dataSource ? (
                <Lookup
        ...
        {paging.enabled == true && <Paging defaultPageSize={10} {...paging} />}

        <Sorting {...sorting} />

        <Pager
          visible={pager.visible !== false}
          allowedPageSizes={pager.allowedPageSizes || [5, 10, 25, 50, 100]}
